    ...
    <div class="p-8 flex flex-col gap-10 min-h-screen relative overflow-hidden bg-gray-50">
          ...
          <div class="flex items-center justify-between">
            <div class="relative">
              <h1 class="text-5xl font-black text-blue-600 mb-3">
                🚀 Data Jenjang AK
                <span v-if="namaInstansi" class="block text-2xl font-bold text-emerald-600 mt-2">{{ namaInstansi }}</span>
              </h1>
              <p v-if="namaInstansi" class="text-lg text-emerald-700 font-semibold mt-1">Data Jenjang AK - {{ namaInstansi }}</p>
      ...
      >
        <div class="relative flex items-center gap-3 bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-4 rounded-full shadow-lg transform transition-all duration-300 hover:scale-105">
          ...
          <span class="hidden sm:block">Update Data</span>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          <h3 class="font-black text-xl text-blue-600 mb-4">
            ...
            <span v-if="namaInstansi" class="text-emerald-600 font-semibold ml-2">({{ namaInstansi }})</span>
          </h3>
          <client-only>
            ...
            <table class="min-w-full text-left border-collapse bg-white/80 backdrop-blur-sm">
              ...
              <tbody>
                <tr v-for="(row, idx) in pagedData" :key="row.nama"
                  ...
                  @click="router.push(`/${username}/form/analis-kebijakan/${row.id}`)"
...
let instansi_id = userStore.instansi_id
let username = userStore.username
if ((!instansi_id || !username) && typeof window !== 'undefined') {
  const user = JSON.parse(localStorage.getItem('user') || '{}')
  if (!instansi_id) instansi_id = localStorage.getItem('instansi_id')
  if (!username) username = user.nama || ''
}
...
onMounted(async () => {
  if (instansi_id) {
    try {
      ...
      const found = Array.isArray(data) ? data.find(i => i.id == instansi_id) : null
...
onMounted(async () => {
  ...
  const res = await fetch(`/api/analis_kebijakan/instansi?instansi_id=${instansi_id}`)
...
async function handleAddData() {
  ...
  errorAdd.value = ""
  try {
    if (!instansi_id) throw new Error("instansi_id tidak ditemukan")
    ...
    // Refresh data
    const r = await fetch(`/api/analis_kebijakan/instansi?instansi_id=${instansi_id}`)
