<template>
		<div class="min-h-screen bg-gray-50 py-8 px-2 md:px-8">
			<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8">
				<h1 class="text-2xl font-bold mb-6 text-blue-700">Input Data Penguji</h1>
				<button class="btn btn-primary mb-6" @click="showModal = true">Tambah Penguji</button>
				<teleport to="body">
					<div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
						<form @submit.prevent="addPenguji" class="bg-white rounded-xl shadow-2xl max-w-md w-full p-8 relative animate-fadein">
							<button type="button" class="btn btn-sm btn-circle btn-ghost absolute right-4 top-4" @click="showModal = false">✕</button>
							<h2 class="text-xl font-bold mb-4 text-blue-700">Tambah Penguji</h2>
							<div class="space-y-4">
								  <input v-model="form.nama" class="input input-bordered w-full" placeholder="Nama Penguji" required />
				...
				<h2 class="text-lg font-semibold mb-2">Daftar Penguji</h2>
				<div class="overflow-x-auto rounded shadow">
					<table class="table w-full text-base">
						...
						<tbody>
							<tr v-for="(p, idx) in pengujiList" :key="idx">
												<td>{{ idx + 1 }}</td>
												<td>
												...
												<td>{{ p.nip }}</td>
												<td>{{ p.instansi }}</td>
												<td><button class="btn btn-error btn-xs" @click="removePenguji(idx)">Hapus</button></td>
							</tr>
						</tbody>
					</table>
				</div>
...
function addPenguji() {
	...
	pengujiList.value.push({ ...form.value })
	form.value = { nama: '', nip: '', instansi: '', foto: '', password: '12345678' }
	showModal.value = false
}
function removePenguji(idx) {
	pengujiList.value.splice(idx, 1)
}
</script>
